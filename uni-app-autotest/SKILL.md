...
17. 若日志主故障为 `spawnSync ... swift-parser-cli ENOENT`、`[plugin:uni:app-uvue-css] ... font-weight` 或 `error: [Circular *1]`，先调用 `hbuilderx-local-build-triage` 归一化签名后再决定修复动作。
...
- `hbuilderx-local-build-triage`：归一化本地 PluginHost/uniapp.test 编译错误（例如 `swift-parser-cli ENOENT`、`app-uvue-css font-weight`）。
